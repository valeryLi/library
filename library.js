...
const libraryWrapper = document.querySelector("#root");
...
libraryWrapper.append(leftDiv, rightDiv);
...
function renderBooksList() {
  ...
  deleteBtns.forEach((elem) => elem.addEventListener("click", deleteBook));
...
function deleteBook(event) {
  const books = JSON.parse(localStorage.getItem("books"));
  const bookID = event.target.parentNode.dataset.id;
  const filterArray = books.filter(({ id }) => id !== bookID);
  localStorage.setItem("books", JSON.stringify(filterArray));
...
function createFormMarkup({ title, author, img, plot }) {
  ...
  <label>Plot:<input tepe='text' name='plot' value='${plot}'/></label>
  <button>Save</button>
  </form>`;
}
